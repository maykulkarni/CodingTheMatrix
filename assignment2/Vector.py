...
def equal(u, v):
    ...
    # compare function
    for keys in u.f.keys():
        if keys not in v.f.keys() or u.f[keys] != v.f[keys]:
            return False
...
def dot(u, v):
    ...
    pass
    ans = 0
    for keys in set(u.f.keys()).intersection(set(v.f.keys())):
        ans += u.f[keys] * v.f[keys]
    return ans
